...
cpi_dk <- get_data(table_id = 'PRIS8', 
...
# currency conversion
eur_dkk <- get_data(table_id = 'DNVALA',
                    variables = list(list(code = 'VALUTA', values = 'EUR'), list(code = 'KURTYP', values = 'KBH'),
...
life_expectancy <- get_data('HISB8', 
                variables = list(list(code = 'ALDER', values = NA), list(code = 'KØN', values = NA),
                                 list(code = 'TAVLE', values = 3),list(code = 'TID', values = NA)))
setDT(life_expectancy)
life_expectancy <- life_expectancy[,.(alder = as.integer(str_remove(ALDER,' year\\w?')), sex = KØN, 

...
lookbacks <- c(1, 3, 5)
dx_vars <- c(
  "dx_cvasc", "dx_copd", "dx_demen", "dx_diabc", "dx_diabuc", "dx_hfail", "dx_hemi",
  "dx_mliver", "dx_mi", "dx_pud", "dx_pvd", "dx_renal", "dx_rheum", "dx_sliver"
)

eq_set <- c(
  paste0("treat ~ sex + age_group + ", outcome_var, " + familie_type + pre_socio + fam_size + edu + reg + ", paste0(dx_vars, collapse = " + ")),
...
clusterExport(cl, c(
  "model_matrix", "model_dir", "treatment_var", "treatment_year", "treatment_match_var",
  "outcome_var"
))
clusterEvalQ(cl, {
  require(MatchIt)
...
clusterApply(cl, years, function(y) {
  ...
  lapply(1:nrow(model_matrix), function(i) {
    ...
    if (y + m$lookbacks <= 2015) {
      ...
      run_matches(
        match_type = m$method, distance_type = m$dist, eqn = m$eq_set,

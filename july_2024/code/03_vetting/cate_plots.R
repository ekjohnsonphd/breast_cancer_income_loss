...
mdf$match_age <- factor(mdf$match_age, levels = c(
  "18to29", "30to34", "35to39", "40to45", "45to49", "50to54", "55to59", "60to64", "65to69",
  "70to74", "75to79", "80to84", "85to89", "90to120"
), ordered = TRUE)
...
mdf_agg <- calculate_att(mdf, groupings = c(
  "match_yr", "match_age", "fam_inc_qt", "familie_type", "fam_size", "pre_socio", "edu", "sbc",
...
cate_plot <- function(mdf, var, yearly = FALSE, relative = FALSE) {
  ...
  g <- ggplot(mdf_stratified) +
    ...
    geom_pointrange(aes(
      x = att, xmin = lower, xmax = upper, y = strat,
  ...
  if (relative) {
    ...
    g <- g + scale_x_continuous(labels = scales::label_currency(
      suffix = NULL, prefix = "€",
...
dtree <- rpart(
  att ~ match_yr + match_age + edu + fam_inc_qt + fam_size + familie_type + pre_socio + dx_cvasc + dx_copd + dx_demen +
...
g6 <- ggplot(mdf_idx) +
  ...
  labs(
    x = "Years since index", y = "Mortality rate (% of total)", color = "Population",

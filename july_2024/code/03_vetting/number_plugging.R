...
## Data and setup ------------------------------------------
med_iqr <- function(x) return(paste0(round(median(x)),' (',round(quantile(x,0.25)),' - ',round(quantile(x,0.75)),')'))

format_results <- function(x) return(paste0(round(x$att),' (',round(x$lower),' - ',round(x$upper),')'))
...
match_df_per <- open_dataset('data/analysis/01_matching/02_matched/dbcg/per_ind/2024-05-25/m13') %>% collect() %>% setDT()
...
match_df_fam <- open_dataset('data/analysis/01_matching/02_matched/dbcg/fam_ind/2024-05-25/m10') %>% collect() %>% setDT()
...
results['10 year personal income loss percent of total'] <- paste0(round(100*cp_nr$att/med_per,2),
                                                                   '% (',round(100*cp_nr$lower/med_per,2),'% - ',round(100*cp_nr$upper/med_per,2),'%)')
results['10 year hhold income loss percent of total'] <- paste0(round(100*ch_nr$att/med_fam,2),

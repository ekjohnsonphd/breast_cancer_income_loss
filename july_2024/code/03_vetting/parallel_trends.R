...
parallel_trends_data <- function(treatment, outcome, dx_y, df, deaths = FALSE) {
  setnames(
    df, c(treatment, paste0(treatment, "_dx_yr"), paste0(outcome, "_ind"), paste0(outcome, "_inc_qt")),
  ...
  sdf <- rbind(
    df[, .(outcome = mean(outcome), mod = "Overall"), by = c("year", "treat")],
  ...
  sdf$mod <- factor(sdf$mod, levels = c(
    "Exposed", "Overall", "Age + sex", "Age + sex + covs",
    "Age + sex + covs + inc"
  ))
  sdf[, index_yr := dx_y]
  return(sdf)
...
m1 <- get_match_time_series(
  match_dir = paste0(model_dir, "m10"), treatment_var, treatment_year, outcome_var, follow_up_length = 10,
  lookback = 1, deaths_as_0 = TRUE, agg = TRUE, prematch = TRUE
)
m2 <- get_match_time_series(
  match_dir = paste0(model_dir, "m11"), treatment_var, treatment_year, outcome_var, follow_up_length = 10,
  lookback = 3, deaths_as_0 = TRUE, agg = TRUE, prematch = TRUE
)
m3 <- get_match_time_series(
  match_dir = paste0(model_dir, "m12"), treatment_var, treatment_year, outcome_var, follow_up_length = 10,
  lookback = 5, deaths_as_0 = TRUE, agg = TRUE, prematch = TRUE
)
m4 <- get_match_time_series(
  match_dir = paste0(model_dir, "m13"), treatment_var, treatment_year, outcome_var, follow_up_length = 10,
  lookback = 1, deaths_as_0 = TRUE, agg = TRUE, prematch = TRUE
)
m5 <- get_match_time_series(
  match_dir = paste0(model_dir, "m14"), treatment_var, treatment_year, outcome_var, follow_up_length = 10,
  lookback = 3, deaths_as_0 = TRUE, agg = TRUE, prematch = TRUE
)
m6 <- get_match_time_series(
  match_dir = paste0(model_dir, "m15"), treatment_var, treatment_year, outcome_var, follow_up_length = 10,
  lookback = 5, deaths_as_0 = TRUE, agg = TRUE, prematch = TRUE
)
m7 <- get_match_time_series(
  match_dir = paste0(model_dir, "m1"), treatment_var, treatment_year, outcome_var, follow_up_length = 10,
  lookback = 1, deaths_as_0 = TRUE, agg = TRUE, prematch = TRUE
)
m8 <- get_match_time_series(
  match_dir = paste0(model_dir, "m2"), treatment_var, treatment_year, outcome_var, follow_up_length = 10,
  lookback = 3, deaths_as_0 = TRUE, agg = TRUE, prematch = TRUE
)
m9 <- get_match_time_series(
  match_dir = paste0(model_dir, "m3"), treatment_var, treatment_year, outcome_var, follow_up_length = 10,
  lookback = 5, deaths_as_0 = TRUE, agg = TRUE, prematch = TRUE
)


dt <- rbind(m1[, model := "m1"], m2[, model := "m2"], m3[, model := "m3"], m4[, model := "m4"], m5[, model := "m5"],
  m6[, model := "m6"], m7[, model := "m7"], m8[, model := "m8"], m9[, model := "m9"],
  fill = TRUE
)

...
st(sumdf, vars = desc_cols, summ = c('mean(x)','median(x)','sd(x)','propNA(x)'), 
   group = 'grp', out = 'csv', 
...
match_groups <- count(match_groups, dbcg, year, age_group, sex, familie_type, ie_type, 
...
ggplot(match_groups) + 
  ...
  theme_bw() +
  labs(x = 'Number of exposed in match group', y = 'Number of controls in match group', title = 'Match group size', 
...
# Correlation tables ------------
cdf <- copy(df)[year == 2015,.(pid, year, familie_type, ie_type, pre_socio, age_group, alder, dbcg, 
